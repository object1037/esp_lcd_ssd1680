...
#define SSD1680_EPD_2IN9_WIDTH         128
#define SSD1680_EPD_2IN9_HEIGHT        296
...
esp_err_t epaper_panel_refresh_screen(esp_lcd_panel_t *panel)
{
    ...
    if (!(epaper_panel->_invert_color)) {
        duc_flag |= SSD1680_PARAM_COLOR_BW_INVERSE_BIT[0];
        duc_flag &= (~SSD1680_PARAM_COLOR_RW_INVERSE_BIT[0]);
    } else {
        duc_flag &= (~SSD1680_PARAM_COLOR_BW_INVERSE_BIT[0]);
        duc_flag |= SSD1680_PARAM_COLOR_RW_INVERSE_BIT[0];
    }
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1680_CMD_DISP_UPDATE_CTRL, (uint8_t[]) {
        (uint8_t[]){duc_flag, 0x80}  // Color invert flag
    }, 2), TAG, "SSD1680_CMD_DISP_UPDATE_CTRL err");
                          ...
                          esp_lcd_panel_handle_t *const ret_panel)
{
    ...
    *ret_panel = &(epaper_panel->base);
    ...
    if (!(epaper_panel->_non_copy_mode)) {
        epaper_panel->_framebuffer = heap_caps_malloc(SSD1680_EPD_2IN9_WIDTH * SSD1680_EPD_2IN9_HEIGHT / 8,
                                     MALLOC_CAP_DMA);
...
esp_err_t epaper_panel_set_bitmap_color(esp_lcd_panel_t *panel, esp_lcd_ssd1680_bitmap_color_t color)
{
    ESP_RETURN_ON_FALSE(panel, ESP_ERR_INVALID_ARG, TAG, "panel handler is NULL");
    epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
...
static esp_err_t epaper_panel_init(esp_lcd_panel_t *panel)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1680_CMD_DISP_UPDATE_CTRL, (uint8_t[]) {
        SSD1680_PARAM_DISP_UPDATE_CTRL
    }, 2), TAG, "SSD1680_CMD_DISP_UPDATE_CTRL err");
    ...
    if ((!(epaper_panel->_mirror_x)) && (!(epaper_panel->_mirror_y))) {
        if (!(epaper_panel->_non_copy_mode)) {
            if (epaper_panel->_swap_xy) {
                memset(epaper_panel->_framebuffer, 0, 128 * 296 / 8);
